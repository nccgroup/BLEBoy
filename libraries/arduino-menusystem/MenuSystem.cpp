...
ConfigMenuItem::ConfigMenuItem(char* configVal, const char* name, SelectFnPtr select_fn)
: MenuItem(name, select_fn){
    _config_value = (char*)malloc(1);
    set_value(configVal);
}

Menu* ConfigMenuItem::select() {
    MenuComponent::select();
    return nullptr;
}

void ConfigMenuItem::reset() {
    // Do nothing.
}

void ConfigMenuItem::render(MenuComponentRenderer const& renderer) const {
    renderer.render_config_menu_item(*this);
}

bool ConfigMenuItem::next(bool loop) {
    return false;
}

bool ConfigMenuItem::prev(bool loop) {
    return false;
}

char* ConfigMenuItem::get_value() const{
    return _config_value;
}
void ConfigMenuItem::set_value(char* value){
    free(_config_value);
    int len = strlen(value);
    //Serial.println(len);
    len += 1;
    _config_value = (char*)malloc(len*sizeof(char));
    strncpy(_config_value, value, len);
    //_config_value[len] = "\0";
